...
export default class TodoApp extends Component {
	...
	buttons = [
		...
		{name: 'Not Completed', label: 'Not Completed'},
	];
	...
	render() {
		...
		const buttons = this.buttons.map(({name, label}) => {
			...
			const clazz = isActive ? styles.filter_pres:styles.filter_button_text;
			return (
				<TouchableOpacity
					key={name}
		...
		return (
			<View style={styles.container}>
				<View
					...
					<View>
						...
						<Text
							...
							You have{' '}
							<Text style={{color: '#9C38FF'}}>{list.length} tasks</Text> to
							complete
						</Text>
					</View>
					<TouchableOpacity
						onLongPress={() => this.resertName()}
						style={{
							width: 50,
							height: 50,
							borderRadius: 50,
							backgroundColor: '#9C38FF',
							alignItems: 'center',
				...
				<View style={[styles.filter_button]}>{buttons}</View>
						...
						renderItem={({item, index}) => {
							return (
								<View
									...
									<TouchableOpacity
										onLongPress={() => this.delete(list, index)}
										key={index}
										onPress={() => {
											this.doneTask(list, index);
										}}
										style={styles.listbox_button}>
										<Text
											style={[
												item.isDone ? styles.done_text : styles.undone_text,
												{
													fontWeight: 'bold',
													color: '#9C38FF',
													paddingLeft: 10,
				...
				<View>
					<TouchableOpacity
						...
						<Text style={{color: '#7512D7', fontWeight: 'bold'}}>
							+ Add a new task
						</Text>
					</TouchableOpacity>
				</View>
					...
					<View style={{flex: 1, justifyContent: 'space-between'}}>
						<View>
							<View
								...
								<TouchableOpacity
									onPress={() => this.visible()}
									style={{marginRight: 30, paddingLeft: 15}}>
